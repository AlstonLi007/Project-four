...
def style_profile_metrics(
    ...
    *,
    ...
    df["is_edited"] = df["is_chosen"] & df["edited_text"].fillna("").str.strip().ne("")
...
def style_profile_before_after(
    ...
    *,
    ...
    df["style_profile_id_int"] = df["style_profile_id"].astype("Int64")
...
def build_contact_summary(memori_df: pd.DataFrame, train_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not memori_df.empty:
        agg_memori = (
            memori_df.groupby("entity_id")
            .agg(
                memori_calls=("entity_id", "size"),
            ...
            .reset_index()
            .rename(columns={"entity_id": "wechat_id"})
        )
    else:
