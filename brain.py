...
from wechat_rag import get_rag_context

OPENAI_MODEL = os.environ.get("OPENAI_MODEL", "gpt-4.1-mini")
USE_LOCAL_NN = os.environ.get("USE_LOCAL_NN", "0") == "1"
LOCAL_NN_DIR = os.environ.get("LOCAL_NN_DIR", "nn_models")
USE_MEMORI = os.environ.get("USE_MEMORI", "1") == "1"
WECHAT_PROCESS_ID = "wechat_auto_reply"
PROMPT_VERSION = "GLOBAL_v1"
...
def _build_prompt(style_profile: str, dialogue: List[ParsedMessage], external_context: str = "") -> str:
...
def _call_llm(
    ...
    *,
    ...
    style_label: str = "default",
    style_profile: Optional[Dict[str, Any]] = None,
    prompt_version: str = PROMPT_VERSION,
    rag_meta: Optional[Dict[str, Any]] = None,
    style_profile_version: Optional[str] = None,
) -> tuple[Optional[List[str]], Optional[str]]:
    """Call OpenAI (prefer Memori-instrumented) and return candidate replies.

    The attribution tags include conversation/message IDs and style metadata so
    Memori analytics can join LLM calls with the originating message and
    profile version.
    """
    ...
    entity = f"wechat:{wechat_id}" if wechat_id else (display_name or "unknown-contact")

    style_profile_id = None
    style_profile_model_id = None
    version = style_profile_version
    if style_profile:
        try:
            style_profile_id = style_profile.get("id")
            style_profile_model_id = style_profile.get("model_id")

            if style_profile_model_id:
                version = str(style_profile_model_id)
            elif style_profile.get("version") is not None:
                version = str(style_profile.get("version"))
    ...
    tags = {
        ...
        "style_profile_model_id": style_profile_model_id,
        "prompt_version": prompt_version,
    }

    if rag_meta:
        for key, value in rag_meta.items():
            tags[f"rag_{key}"] = value
    ...
    if USE_MEMORI:
        try:
            client = get_openai_client(
                entity_id=entity,
                process_id=WECHAT_PROCESS_ID,
                tags=tags,
            )
        except Exception as exc:  # pragma: no cover - Memori may be absent or misconfigured
            print("[brain] Memori integration failed, falling back to raw OpenAI:", repr(exc))
...
def draft_reply(
    ...
    *,
    ...
    style_label = "default"
    profile = db.get_style_profile(contact_id=contact.id, channel=channel, label=style_label)
    style_prompt = profile.get("prompt_text") if profile else ""
    style_profile_version: Optional[str] = None
    if profile:
        if profile.get("model_id"):
            style_profile_version = str(profile.get("model_id"))
        elif profile.get("version") is not None:
            style_profile_version = str(profile.get("version"))
    ...
    rag_mode = db.get_contact_rag_mode(contact.id, default=os.environ.get("WECHAT_RAG_MODE", "off"))
    rag_half_life_days = db.get_contact_half_life(
        contact.id, default=float(os.environ.get("WECHAT_RAG_HALF_LIFE_DAYS", 30.0))
    )
    if latest_text:
        try:
            needs_knowledge = ("?" in latest_text) or ("？" in latest_text) or (len(latest_text) > 40)
            if needs_knowledge:
                rag_context, rag_meta = get_rag_context(
                    ...
                    rag_mode=rag_mode,
                    half_life_days=rag_half_life_days,
                )
        except Exception as exc:
            print("[brain] RAG 查询失败，忽略:", repr(exc))
            rag_context, rag_meta = "", {"enabled": False, "error": repr(exc)}

    llm_prompt = _build_prompt(style_prompt, messages, external_context=rag_context)
    ...
    if not replies:
        db.log_llm_failover(
            ...
            backend="openai",
            error_message=llm_error or "llm_unavailable",
            heuristic_name="_heuristic_replies",
        )
